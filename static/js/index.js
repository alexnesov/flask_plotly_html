...
function fetchBarGraphJsonData() {
    // Graph_1
    console.log("sending request for > barGraph < to server");
    fetchGraphData("http://127.0.0.1:5000/api/fetchBarGraphJsonData", "barGraph", "loader_graph1", "refreshBtn_1", "graph_1_TS");
}
...
function fetchTempMock() {
    // Graph_3
    console.log("sending request for > temp < to server");
    fetchGraphData("http://127.0.0.1:5000/api/mockDelay", "temp", "loader_graph3", "refreshBtn_3", "graph_3_TS");
}


// Initial Plots
Plotly.newPlot("barGraph", null, { autosize: true }, { responsive: true }); // Graph 1
Plotly.newPlot("signalChart", null, { autosize: true }, { responsive: true }); // Graph 2

fetchBarGraphJsonData(); // inital page load graph 1
fetchSignalChartJsonData(); // inital page load graph 2

Plotly.newPlot("temp", null, { autosize: true }, { responsive: true }); // Graph 3
fetchTempMock();
